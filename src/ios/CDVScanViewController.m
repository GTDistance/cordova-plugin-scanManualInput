...
#define SCANTEXT          @"请对准包装、说明书或设备上的二维码"
...
- (void)setupMaskView{
    ...
    inputBtn.frame = CGRectMake((SCREENWidth-120)/2,SCREENHeight*0.9-kBorderW*2+80, 120, 30);
    [inputBtn setTitle:@"手动输入 >" forState:UIControlStateNormal];
    inputBtn.contentMode = UIViewContentModeScaleAspectFit;
...
- (void)inputAction:(UIButton *)button{
    [[NSNotificationCenter defaultCenter] postNotificationName:@"scan" object:self userInfo:@{@"content":@"手动输入"}];
    [self handleClose:NULL];
}
